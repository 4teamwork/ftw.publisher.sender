...
class TestExtractor(PloneTestCase):
    ...
    def afterSetUp(self):
        ...
        topicid = self.folder.manage_addProduct['ATContentTypes'].addATTopic(id='topic1',title='topic 1')
    ...
    def test_extractor_object(self):
        ...
        expected_adapters = [u'field_data_adapter',u'properties_data_adapter',u'interface_data_adapter',u'portlet_data_adapter']
        list_adapters = []
        for name,adapter in adapters:
            list_adapters.append(name)
        self.assertEquals(expected_adapters,list_adapters)

    def test_extractor_topic(self):
        adapters = getAdapters((self.topic1,),IDataCollector)
        expected_adapters = [u'field_data_adapter',u'properties_data_adapter',u'topic_critera_adapter',u'interface_data_adapter',u'portlet_data_adapter']
        list_adapters = []
        for name,adapter in adapters:
            list_adapters.append(name)
        self.assertEquals(expected_adapters,list_adapters)
    ...
    def test_getMetadata(self):
        ...
        uid=metadata['UID']
        expected_metadata = {'UID': uid,
                             'portal_type': 'Document',
                             'physicalPath': '/Members/test_user_1_/test-doc-1', 
                             'sibling_positions': {'topic1': 1, 'test-doc-1': 0},
                             'action': 'delete',
